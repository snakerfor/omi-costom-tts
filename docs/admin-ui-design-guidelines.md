...
- 不应把 Soniox 标签作为主要搜索字段。Soniox 标签只是转录服务生成的临时 speaker label，不应被当成可信身份。
- 不应把“发言人姓名”和 “Soniox 标签”混在一个搜索框里。正式发言人筛选应基于系统中的 official speaker，而不是 Soniox label。

说明：

- 按正式发言人筛选是必要能力。用户需要定位某个人最近参与过哪些会话、说过哪些片段，以及这些片段在不同会话中的分布。
- 按发言人身份筛选和按正式发言人筛选是两个不同维度：身份用于找一类人，例如客户、本人、同事；正式发言人用于找具体某个人。
- Transcript 文本关键词搜索仍然是主搜索入口，用于从所有对话中查找具体内容。
...
入口设计不应被固定为“必须在当前页面内嵌所有字段”。可以使用顶部工具栏、底部 sticky 操作条、弹窗、抽屉或组合式流程。

推荐交互：

1. 用户先在 transcript 中勾选片段。
2. 页面显示一个清晰的批量操作入口，例如 `加入候选语料`。
3. 点击后打开弹窗或右侧抽屉。
4. 在弹窗/抽屉中选择目标模式，并完成已有发言人选择或新建发言人信息填写。
5. 用户确认后，才写入本地候选语料。
...
- 如果字段较少，可以使用轻量弹窗。
- 如果后续新建发言人字段变多，应优先考虑右侧抽屉，避免弹窗过高。
- Transcript 主页面不应长期堆放新建发言人的所有字段，否则会破坏 transcript 核对区的信息密度。
- 批量入口应显示已选片段数量、总时长，并提示是否包含不可加入片段。
- 用户确认前，应明确提示该操作只加入本地候选语料，不会更新讯飞。
